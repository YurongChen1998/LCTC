...
def show_rgbimg(data, savepath="./Results/current_RGB.png"):
    # data shape: [c, h, w]
    img = data.detach().cpu().numpy()
    rgb_reco = np.zeros((img.shape[1], img.shape[2], 3), dtype=np.float32)
    rgb_reco[...,0], rgb_reco[...,1], rgb_reco[...,2] = img[-1, :, :], img[5, :, :], img[0,:, :]
    img_hist, bins = np.histogram(rgb_reco.flatten(), 256, density=True)
    cdf = img_hist.cumsum()
    cdf = 1. * cdf / cdf[-1]
    img_eq = np.interp(rgb_reco.flatten(), bins[:-1], cdf)
    rgb_reco = img_eq.reshape(rgb_reco.shape)
    hv.extension("matplotlib", logo=False)
    rgb_reco_hv = hv.RGB((np.arange(rgb_reco.shape[1]),np.arange(rgb_reco.shape[0]),rgb_reco[:,:,0],rgb_reco[:,:,1],rgb_reco[:,:,2]))
    rgb_reco_hv.opts(fig_inches=22).opts(xlabel="along-track",ylabel="cross-track",invert_yaxis=True)
    hv.save(rgb_reco_hv, savepath, fmt='png')